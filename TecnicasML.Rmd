...
#knitr::write_bib(x=c("caret","ggplot2","rpart", "randomForest", "gbm", "xgboost"), file = "knitcitations.bib")
...
Una vez ajustadas las técnicas de árboles, he ajustado otra vez los modelos con *validación cruzada repetida* esta vez, para comparar las respectivas *tasa de fallo* y la *area abajo la curva ROC*, a través de un grafo de caja y bigote incluyendo como modelo de referencia, una *regresión logística*.

Una vez encontrado el modelo ganador, he decidido crear un grafo de barras apiladas para obtener las variables más influyentes según los distintos modelos. A partir de estas, he creado un nuevo conjunto *entrenamiento/test*, con el cúal voy a realizar otras técnicas de aprendizaje automatico:

- Suppor Vector Machine, con kernel:
    - Líneal
    - Polynomial
    - Radial
- Bagging del SVM Líneal
- Boosting
- Stacking
...
### Nuevo conjunto *Test* con variables más influyentes {-}
...
plot(svm_lin_result, output = "ggplot")
```
...
roc(svm_lin$pred$obs,
    ...
    base_family = "Times", base_size = 10) + labs(title = "ROC Curve", subtitle = "SVM Lineal") +
...
modelLookup("svmPoly")[2:3] %>%
    cbind(description = linebreak(c(
    	"Degree needed for the kernel of type polynomial.",
      " .",
...
plot(svm_poly_result, output = "ggplot")
```
...
returning the predictions for the test data for each of the trees
...
list(
	...
	scale_color_manual(name = NULL,
										 values = c(`Bagging SVM` = "#2E45B8",
										 					 SVM = "#9EA9E0")) + 
	hrbrthemes::theme_ipsum(
		ticks = T, base_family = "Times", base_size = 10) + labs(title = "ROC Curve", subtitle = "Comparing ROC curves for single models and Stacking") +
...
boost_tterr <- as_data_frame(cbind(
	test_error=errorevol(boost, Test)$error,
	train_error=errorevol(boost, upTrain2)$error))
	apexchart(
		width = 800,
		ax_opts = list(
			chart = list(type = "line"),
			...
			markers = list(style = "inverted", size = 4),
			series = list(
				list(name = "Accuracy", data = boost_tterr$test_error),
				list(name = "AUC", data = boost_tterr$train_error)),
			title = list(text = "Training Results",
									 align = "center"),
			xaxis = list(categories = seq(0, 3, by = 0.1)))) %>%
		ax_yaxis(labels = list(formatter = format_num(".0%"))) %>%
		ax_stroke(width = 1) %>% 
	ax_colors("#4B69CE", "#B0BDE9")
```
...
Los stacking no han aportado mejorias a los modelo singolos
